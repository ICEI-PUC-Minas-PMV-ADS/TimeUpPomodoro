    ...
    <View style={[styles.note, { width: windowWidth - 40 }]}>
      <View style={styles.noteHeader}>
        {data.item ? (<TextInput
          ...
          maxLength={20}
          onChangeText={(text) => { handleChangeTitle(text, data.item) }}
        ...
        {data.item ? (
          <>
            ...
            <TouchableOpacity onPress={() => handlePinned(data.item.id)}>
              <Icon name={data.item.pinned ? 'pin-off-outline' : 'pin-outline'} size={30} color="#292B2C" />
            </TouchableOpacity>
            {/* delete */}
            <TouchableOpacity onPress={() => handleDelete(data.item.id)} style={{ marginLeft: 5 }}>
          ...
          <>
            ...
            {/* menu */}
            <TouchableOpacity onPress={() => handleOpenList()} style={{ marginLeft: 5 }}>
      ...
      {data.item ? (<TextInput
        ...
        maxLength={250}
        onChangeText={(text) => { handleChangeDescription(text, data.item) }}
        multiline
